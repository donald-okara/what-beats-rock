...
object Prompts {
    ...
    fun buildStrictChatAnswerPrompt(pastResponses: List<String>, newResponse: String): String {
        return buildString {
            ...
            appendLine()
            appendLine("- 🟡 If the response is logical but not creative (e.g. 'rock beats scissors'), you may award 1 point for correctness.")
            appendLine("    ➤ Justify it as 'technically correct but unoriginal' or similar.")
            appendLine("    ➤ Feel free to lightly roast it if it's safe but dull. A little sass is encouraged 😏.")
            appendLine()
